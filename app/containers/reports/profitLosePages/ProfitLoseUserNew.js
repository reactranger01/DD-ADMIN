...
function ProfitLoseUserNew() {
  ...
  }, [rowCountState, paginationModel, setRowCountState, eventid, market]);
    ...
    if (islogin) {
      try {
        const response = await getAuthData(
          `/user/all-user/${market}/${eventid}?limit=8&offset=${
        ...
        if (response?.status === 201 || response?.status === 200) {
          ...
          const formattedData = response.data.data.map((entry, index) => {
            const profit =
              ...
              Number(entry.all_user_losing_amount);
  ...
  const CasinoAviatorcolumns = [
    {
      ...
      renderCell: (params) => (
        ...
        >
          <Link
            to={`/report_eventprofit_bet_history/${params.row.username}/${market}/${params.row.event}/${startDate}/${endDate}`}
          >
            <h6 className="text-blue-400">{params.row.username}</h6>
          </Link>
        </div>
      ),
    },
...
export default ProfitLoseUserNew;
