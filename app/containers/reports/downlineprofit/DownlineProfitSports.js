...
function DownlineProfitSports() {
  ...
  const { path, usetype, startDate, endDate } = useParams();
    ...
    if (islogin) {
      try {
        const response = await getAuthData(
          `/user/downline-user?path=${path}&username=${usetype}&limit=8&offset=${
          ...
          <Link
            to={
              params.row.user_type == 'MASTER'
                ? `/report/downline-profit-sports-user/${params.row.path}/${params.row.user_type}`
  ...
  return (
    <>
      ...
      {isLoading && <Loading />}
      <div>
        <div className="mt-4 h-24 mx-8  border-solid border border-slate-950 rounded-md hidden bg-slate-100  text-sm text-slate-950 ">
...
export default DownlineProfitSports;
