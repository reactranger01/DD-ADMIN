...
const DownlineProfitLoseCasinoAviatorMarket = () => {
  ...
  useEffect(() => {
    const [userIdPart, gameIdPart] = id
      .split('-')
  ...
  const CasinoAviatorcolumns = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        ...
        >
          <Link
            to={`/user_account_aviator/${userid}-${gameId}-${params.row.event.replace(
              /\./g,
              '-',
            )}/downline_proividergames_casinoaviator/${startDate}/${endDate}`}
          >
...
export default DownlineProfitLoseCasinoAviatorMarket;
