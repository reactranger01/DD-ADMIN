...
const UserbookChild = ({ isopenUserBookchild, handlecloseuserchild }) => {
  return (
    <div>
      <div
        className={
      ...
      >
        <div className="text-sm text-white  text-center h-4 rounded-t-lg bg-[#040B1D] relative w-full max-w-lg mx-auto shadow-md px-8 py-4 row flex justify-between items-center">
          <h4 className="text-sm font-bold">USER Book</h4>
          <button className=" flex-none">
            <RxCross2 className="h-4 w-4 " onClick={handlecloseuserchild} />
          </button>
        </div>
...
UserbookChild.propTypes = {
  isopenUserBookchild: PropTypes.bool.isRequired,
  handlecloseuserchild: PropTypes.func.isRequired,
};

export default UserbookChild;
