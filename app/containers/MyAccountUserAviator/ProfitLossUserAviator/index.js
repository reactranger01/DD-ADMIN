...
function ProfitLossUserAviator() {
  const { id } = useParams();
  ...
  }, [rowCountState, paginationModel, setRowCountState, userid, username]);
  ...
  const getDownlineUserList = async () => {
    ...
    if (islogin && userInfo) {
      try {
        const response = await getAuthData(
          `/user/user-casino?limit=8&offset=${
  ...
  const columns = [
    {
      ...
      renderCell: (params) => (
        ...
        >
          <Link
            to={`/user_account_aviator/${id}-${
              (params.row.event_type == 'casino' && 3) ||
              (params.row.event_type == 'Aviator' && 5) ||
              params.row.game_id
            }/downline_profitlose_casinoaviator_market/${dayjs(
              selectedFromDate,
            ).format('YYYY-MM-DD')}/${dayjs(selectedToDate)
              .add(1, 'day')
              .format('YYYY-MM-DD')}`}
          >
...
export default ProfitLossUserAviator;
