...
const CreditRefLog = ({ isCreditRefLog, handlecloseCreditRefLog }) => {
  const row = [
    {
      id: 1,
      formname: 'Upline',
    ...
    >
      <div className="text-sm text-white font-bold text-center h-4 rounded-t-lg bg-[#071535] relative w-full max-w-6xl mx-auto shadow-md px-8 py-4 row flex">
        <h4 className="mt-[-0.5rem] grow text-left">
          Credit Reference Log - mumka15
        </h4>
        <button className=" flex-none">
          <RxCross2
            className="h-4 w-4 mt-[-0.5rem]   "
            onClick={handlecloseCreditRefLog}
          />
        </button>
      </div>
...
CreditRefLog.propTypes = {
  isCreditRefLog: PropTypes.bool.isRequired,
  handlecloseCreditRefLog: PropTypes.func.isRequired,
};
export default CreditRefLog;
